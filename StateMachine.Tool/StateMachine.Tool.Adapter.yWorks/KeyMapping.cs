...
namespace StateMachine.Tool.Adapter.yWorks
{
    public struct Key
    {
        ...
        public Key(int line, int pos, string id, string attrName, string attrType, string fileType, string forValue,
            string defaultValue)
        {
            Line = line;
            Position = pos;
    ...
    public class KeyMapping : IReadOnlyCollection<Key>
    {
        ...
        public KeyMapping(XElement element)
        {
            ...
            foreach (var key in keyElements)
            {
                ...
                keyList.Add(new Key(((IXmlLineInfo) key).LineNumber, ((IXmlLineInfo) key).LinePosition, id, attrName,
                    attrType, fileType, forValue, defaultValue));
            }

            keys = keyList.ToArray();

            GraphDescrionId = keys.FirstOrDefault(k =>
                k.For == "graph" && k.AttributeName == "description" && k.AttributeType == "string").Id;
            NodeDescrionId = keys.FirstOrDefault(k =>
                k.For == "node" && k.AttributeName == "description" && k.AttributeType == "string").Id;
            EdgeDescrionId = keys.FirstOrDefault(k =>
                k.For == "edge" && k.AttributeName == "description" && k.AttributeType == "string").Id;
            ...
            var initialKey = keys.FirstOrDefault(k =>
                k.For == "node" && k.AttributeName == Constants.IsInitialStatePropertyName.ToLowerInvariant() &&
                k.AttributeType == "int");
            if (initialKey.IsValid)
            {
                ...
                if (initialKey.DefaultValue != null)
                    if (int.TryParse(initialKey.DefaultValue, out value) == false)
                        throw new FormatException(string.Format("Invalid default value at line {0} position {1}.",
                            initialKey.Line, initialKey.Position));
                InitialStateDefaultValue = value != 0;
            }

            var finalKey = keys.FirstOrDefault(k =>
                k.For == "node" && k.AttributeName == Constants.IsFinalStatePropertyName.ToLowerInvariant() &&
                k.AttributeType == "int");
            if (finalKey.IsValid)
            {
                ...
                if (finalKey.DefaultValue != null)
                    if (int.TryParse(finalKey.DefaultValue, out value) == false)
                        throw new FormatException(string.Format("Invalid default value at line {0} position {1}.",
                            finalKey.Line, finalKey.Position));
